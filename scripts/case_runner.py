...
def load_case_yaml(path: Path) -> dict[str, Any]:
    ...
    for wing_name, wing in raw["wings"].items():
        ...
        if "beta" in kin and "mean" not in kin["beta"]:
            raise ValueError(f"Missing wings.{wing_name}.kinematics.beta.mean")
...
def _harmonics_to_motion(
    ...
    """
    motion: dict[str, Any] = {}
    for src_angle, motion_prefix in (("gamma", "gamma"), ("phi", "phi"), ("psi", "psi")):
        entry = kinematics[src_angle]
        mean_rad = math.radians(float(entry["mean"]))
        motion[f"{motion_prefix}_mean"] = mean_rad
        ...
        motion[f"{motion_prefix}_amp"] = amp_rad
        motion[f"{motion_prefix}_phase"] = phase_rad

    if "beta" in kinematics:
        entry = kinematics["beta"]
        motion["cone_mean"] = math.radians(float(entry["mean"]))
        pairs = entry.get("harmonics", [])
        cone_amp: list[float] = []
        cone_phase: list[float] = []
        for amp_deg, phase_deg in pairs:
            a = float(amp_deg)
            p = float(phase_deg)
            if a < 0:
                a = -a
                p += 180.0
            cone_amp.append(math.radians(a))
            p_rad = math.radians(p)
            cone_phase.append(math.atan2(math.sin(p_rad), math.cos(p_rad)))
        while len(cone_amp) < n_harmonics:
            cone_amp.append(0.0)
            cone_phase.append(0.0)
        motion["cone_amp"] = cone_amp
        motion["cone_phase"] = cone_phase
...
def _max_harmonics(case: dict[str, Any]) -> int:
    ...
    for wing in case["wings"].values():
        for angle in ("gamma", "phi", "psi", "beta"):
            if angle not in wing["kinematics"]:
                continue
...
def yaml_to_cfg(case: dict[str, Any], source_path: Path) -> str:
    ...
    have_cone_motion = any("cone_mean" in wing_motions[n] for n in names)
    cone_mean_global = (
        sum(wing_motions[n].get("cone_mean", 0.0) for n in names) / len(names)
        if have_cone_motion else 0.0
    )
    ...
    cone_mean_line = f"\ncone_mean = {fmt(cone_mean_global)}" if have_cone_motion else ""
...
psi_mean = {fmt(psi_mean_global)}{cone_mean_line}
