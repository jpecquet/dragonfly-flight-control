...
The hybrid renderer loads `assets/dragonfly_body.obj` for the body mesh.

Wing meshes are generated procedurally (no wing `.obj` required) using a
composite-ellipse planform about the 1/4-chord pitching axis:
- local +X: chord direction (`e_c`)
- local -Y: span direction (root -> tip, then mirrored in transform)
- default plotting aspect ratio: `L^2 / S = 8`
- full-ellipse span envelope: `sqrt(1 - (2*eta - 1)^2)`
- leading-edge offset: `+c/4 * envelope`
- trailing-edge offset: `-3c/4 * envelope`
- `eta` is normalized span station in `[0,1]`
