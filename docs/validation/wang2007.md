...
The vertical projection of the total aerodynamic force, normalized by the dragonfly weight, was computed and is compared to the result from {cite}`wang2007`, obtained by extrapolating 2D CFD results to 3D using blade element theory, in Fig. 5. The mean nondimensional force over one wingbeat is 1.03 for the model and 0.81 for the CFD. There is general agreement for the set of parameters used. Shown as dashed lines is the force for masses corresponding to the 68% prediction interval from the Wakeling forewing-span regression (approximately 0.78x and 1.29x the nominal mass), which is the parameter with the greatest uncertainty.
