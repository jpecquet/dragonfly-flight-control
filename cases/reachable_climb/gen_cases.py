#!/usr/bin/env python3
"""Select equilibrium solutions for slow vertical climb (ux=0, uz=1) and write case YAMLs + post.yaml.

Run once manually:
    python cases/reachable_climb/gen_cases.py

Produces:
    cases/reachable_climb/case_0_sync.yaml ... case_N_sync.yaml
    cases/reachable_climb/post.yaml
"""
...
H5_PATH = REPO_ROOT / "docs/research/reachable/artifacts/reachable_climb_slow.h5"
OUT_DIR = Path(__file__).resolve().parent

# Physical morphology — identical to reachable_hover cases.
BODY_LENGTH_M = 0.04
GRAVITY = 9.81
MU0 = 0.025
LB0 = 0.75
...
def _write_case_yaml(branch_idx: int, params: np.ndarray, ux_val: float, uz_val: float) -> Path:
    """Write a sync case YAML (all wings same phase) for one climb branch."""
    ...
    case = {
        "case_id": f"reachable_climb_{branch_idx}_sync",
        "description": f"Reachable slow climb equilibrium branch {branch_idx} (sync)",
        "specimen": {
...
def _write_post_yaml(n_branches: int) -> None:
    sync_cases = [f"cases/reachable_climb/case_{i}_sync.yaml" for i in range(n_branches)]
    ...
    for i in range(n_branches):
        artifacts.append({
            ...
            "output": f"reachable_climb_{i}_panel.{{theme}}.png",
            "n_samples": 10,
            "stroke_lim": 0.5,
            "force_scale": 0.06,
        })
    ...
    config = {
        ...
        "simulation": {
            ...
            "cases": sync_cases,
            "run_dir": "docs/research/reachable/artifacts/climb_sync",
...
def main() -> None:
    ...
    with h5py.File(H5_PATH) as f:
        ...
        ux_val, uz_val = float(ux[0]), float(uz[0])
        print(
            f"Grid point: ux={ux_val:.4f}, uz={uz_val:.4f} (nondim), "
            f"min_res={min_res[0,0]:.2e}"
        )
        print(f"  params: {param_names}")

        raw_branches: list[np.ndarray] = []
        for k in range(len(f["branches"])):
            res = float(f[f"branches/{k}/residual"][0, 0])
            if res < tol:
                p = f[f"branches/{k}/params"][0, 0, :]
                raw_branches.append(np.array(p))
                print(f"  branch {k}: residual={res:.2e}  params={p}")

    unique_branches = _deduplicate_params(raw_branches)
    unique_branches.sort(key=lambda p: p[1])  # ascending flapping amplitude
    ...
    for i, params in enumerate(unique_branches):
        ...
        psi_phase_deg = math.degrees(params[3])
        print(
            f"  {i}: $\\phi_1 = {phi_amp_deg:.1f}^\\circ$, "
            f"$\\psi_1 = {psi_amp_deg:.1f}^\\circ$, "
            f"$\\delta_\\psi = {psi_phase_deg:.1f}^\\circ$"
        )

    print("\nDone. Run:")
    print("  python -m scripts.docs_media_runner cases/reachable_climb/post.yaml")
