#!/usr/bin/env python3
"""Select equilibrium solutions for slow forward climb (ux=1, uz=1) and write case YAMLs + post.yaml.

Run once manually:
    python cases/reachable_climb_forward/gen_cases.py

Produces:
    cases/reachable_climb_forward/case_0_sync.yaml ... case_N_sync.yaml
    cases/reachable_climb_forward/post.yaml
"""
...
H5_PATH = REPO_ROOT / "docs/research/reachable/artifacts/reachable_climb_forward.h5"
...
def _write_case_yaml(branch_idx: int, params: np.ndarray, ux_val: float, uz_val: float) -> Path:
    """Write a sync case YAML (all wings same phase) for one forward-climb branch."""
    ...
    case = {
        "case_id": f"reachable_climb_forward_{branch_idx}_sync",
        "description": f"Reachable forward climb equilibrium branch {branch_idx} (sync)",
        "specimen": {
...
def _write_post_yaml(n_branches: int) -> None:
    sync_cases = [f"cases/reachable_climb_forward/case_{i}_sync.yaml" for i in range(n_branches)]
    ...
    for i in range(n_branches):
        artifacts.append({
            ...
            "output": f"reachable_climb_forward_{i}_panel.{{theme}}.png",
            "n_samples": 10,
            "stroke_lim": 0.5,
            "force_scale": 0.06,
        })
    ...
    config = {
        ...
        "simulation": {
            ...
            "cases": sync_cases,
            "run_dir": "docs/research/reachable/artifacts/climb_forward_sync",
...
def main() -> None:
    ...
    print("\nDone. Run:")
    print("  python -m scripts.docs_media_runner cases/reachable_climb_forward/post.yaml")
